...
class RollingHorizon():
    ...
    def correction(self, pred_image, headed_pred_image, mode):
        GHOST = 6
        PACMAN = 1
        ...
        if (pred_image == GHOST).sum().item() == 0:
            # Get positions where ghost is located in those heads where it's present
            positions = (headed_pred_image == GHOST).nonzero()
            if 'sampled' in mode:
                # Sample a position
                try:
                    self.pos['g'] = np.random.choice(positions[:, 1].cpu())
                except:
                    self.pos['g'] = self.pos['cg']
            elif 'vote' or 'compo' in mode:
                try:
                    self.pos['g'] = torch.mode(positions[:, 1])[0].item()
                except:
                    self.pos['g'] = self.pos['cg'] #g_curr_pos
            # Place the ghost
            pred_image[self.pos['g']] = GHOST

        if (pred_image == PACMAN).sum().item() == 0:
            # Get positions where ghost is located in those where it's present
            positions = (headed_pred_image == PACMAN).nonzero()
            if 'sampled' in mode:
                # Sample a position
                try:
                    self.pos['p'] = np.random.choice(positions[:, 1].cpu())
                except:
                    self.pos['p'] = self.pos['cp']
            elif 'vote' or 'compo' in mode:
                try:
                    self.pos['p'] = torch.mode(positions[:, 1])[0].item()
                except:
                    self.pos['p'] = self.pos['cp'] #p_curr_pos
            # Place the ghost
            pred_image[self.pos['p']] = PACMAN
        ...
        for e in range(p.N_EVALS):
            ...
            for step in range(p.SEQ_LEN):
                ...
                if 'avg' in type:
                    ...
                    if p.ERROR_CORRECTION:
                        ec_image = self.correction(avg_pred_state, pred_state, 'compo')
                    ...
                    rollout_reward += pred_reward
                    if p.ERROR_CORRECTION:
                        ec_image = self.correction(vote_pred_state, pred_state, 'vote')
                        self.trajecECImg.append(join(trajDir + 'simstateEC' + str(step) + 'eval' + str(e) + '.png'))
                        prepAndSaveImg(ec_image, trajDir, join('simstateEC' + str(step) + 'eval' + str(e) + '.png'))
                        imagined_imageEC = mpu.target_to_pix(ec_image.data.cpu().numpy())
                    ...
                    rollout_reward += pred_reward
                    if p.ERROR_CORRECTION:
                        ec_image = self.correction(samp_pred_state, pred_state, 'sampled')
                        self.trajecECImg.append(join(trajDir+'simstateEC'+str(step)+'eval'+str(e)+'.png'))
                        prepAndSaveImg(ec_image, trajDir, join('simstateEC'+str(step)+'eval'+str(e)+'.png'))
                        imagined_imageEC = mpu.target_to_pix(ec_image.data.cpu().numpy())
