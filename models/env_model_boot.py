...
from networks.modelEnvBootstrapped import EnvModelBootstrappedNetwork
...
class EnvModel():
    ...
    def imagine(self, x, head_idxs):
        #states, rewards = self.envNet(x, head_idxs)
        #return states, rewards
        observations, rewards = self.envNet(x, head_idxs)
    ...
    def learn(self, ir, ts, tr, state, action, m, action_space, input_space):
        ...
        for head in range(p.NUM_HEADS):
            ...
            #SHIET now i need to get batch instances to anulate them with the masks
