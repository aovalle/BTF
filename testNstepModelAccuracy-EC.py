...
fname = './worldModels/env_model_boot_a2c_50k'
env_model.load_state_dict(fname)
fname = './worldModels/env_model_rpf_a2c_50k'
...
while episode < EPISODES:
    ...
    # TODO: WHAT SHOULD BE g_currpos default?
    try:
        g_curr_pos = env.get_ghost_pos()
        g_curr_pos = 15 * g_curr_pos[0] + g_curr_pos[1]
    except:
    ...
    for ste in range(SEQUENCE):
        ...
        if not RPF:
            ...
            if BOOT_TYPE[BT] == 'compo':
                ...
                im_obs_rpf = torch.tensor(
                    np.choose(np.random.randint(imagined_image_rpf.shape[0], size=imagined_image_rpf.shape[1]),
    ...
    #NOTE: I think easiest solution is to just run individually
    # two parms controlling type and whether EC shoul be used
    # think about the implications on the difference of number of frames
    # IDEA: check max frame ceiling on 100 episodes and use that to decide when to cut
    # either at 100 episodes or at certain # of frames

    for ste in range(SEQUENCE):
        # ==== Real World
        state, reward, done, _ = env.step(actions[ste])
        state_torch = torch.FloatTensor(state).unsqueeze(0)
        rewardsEpi += reward
    ...
    # Compute accuracies
    nistates = np.array(istates[:ste + 1])
    nrstates = np.array(rstates[:ste + 1])
    total_acc_obs.append((np.array(istates[:ste + 1]) == np.array(rstates[:ste + 1])).mean())
    nirew = np.array(irew[:ste + 1])
    nrrew = np.array(rrew[:ste + 1])
